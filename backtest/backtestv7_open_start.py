...
from backtestv1_functions import initialize_csv, get_active_sessions, get_highest_high, get_highest_high_lowest_low, get_lowest_low, get_market_trendline_slope, get_mean_close, get_vwap, load_data_from_file, convert_to_pht, get_rsi, get_atr
...
for kline in data:
    ...
    # **Trading Logic:**
    if pht_time.hour == 10 and pht_time.minute == 0 and current_position_data['stage'] == 0:  # 10 AM PHT
    ...
    if current_position_data['position'] == 'long' and close <= current_position_data['stoploss']:
        ...
        # Update pnl regardless of whether it's a win or loss
        current_position_data['pnl'] += pnl_percentage_after_fee  # Add loss or gain to running PnL

        if pnl_percentage_after_fee > 0:
            print(f"Stop-loss hit for LONG position. Profitable PnL: {pnl_percentage_after_fee}%")
        else:
    ...
    if pht_time.hour >= 15 or pht_time.hour < 5:  # Time between 15:00 and 5:00
        ...
        if current_position_data['position'] == 'long' and kline_data['rsi'] > 70:
            ...
            # Update pnl based on profit or loss
            current_position_data['pnl'] += pnl_percentage_after_fee
            ...
            if pnl_percentage_after_fee > 0:
                ...
                print("Recorded Win for LONG position")
            else:
                # Record losing trade
                print("Recorded Loss for LONG position")
            ...
            if pnl_percentage_after_fee > 0:
                # Record winning trade
                print("Recorded Win for SHORT position")
            else:
                # Record losing trade
                print("Recorded Loss for SHORT position")
